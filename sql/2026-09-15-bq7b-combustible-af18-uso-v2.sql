-- 2026-09-15-bq7b-combustible-af18-uso-v2.sql
-- BQ7b — registrar_combustible_app: el gate AF18 (DR481 "solo el usuario asignado")
-- unía SOLO vehiculo_asignaciones + responsable_id; ahora TAMBIÉN honra la sesión
-- uso-v2 activa (vehiculo_usos.fin_at is null) del que registra. Aditivo/retrocompat:
-- solo amplía quién puede registrar; no rechaza ningún caso que antes pasaba. Copia
-- VIVA del RPC (pg_get_functiondef) con el único cambio en la condición del raise.
-- Regla 14 (gate por rol/tenencia en un solo predicado) + corolario BO2.
...
begin
  ...
  if not v_persona then
    ...
    if not sgc.es_flota_elevado() then
      ...
      -- BQ7b — el gate AF18 miraba SOLO vehiculo_asignaciones/responsable_id e
      -- ignoraba la sesión uso-v2: un chofer que RECIBIÓ el vehículo (vehiculo_usos
      -- activo) no podía echarle gasolina (DR481). Se une el uso activo (misma clase
      -- que BO2, pero server-side). Aditivo: solo AFLOJA el gate, no bloquea nada nuevo.
