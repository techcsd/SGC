-- ============================================================================
-- AD7 (parte B) — Ronda 31/07/2026 (PROMPT-15 FASE 3)
-- Reemplaza registrar_combustible_app (MISMA firma de 19 args) para clasificar
-- en 4 estados vía sgc.clasificar_rendimiento, con baseline SIN envenenar.
-- + recalcular_estados_combustible() (histórico) + set_flota_config() (admin).
-- Depende de 2026-07-31-ad7-rendimiento-combustible-calibrado.sql.
-- ============================================================================
...
begin
  ...
  if not v_persona then
    ...
    -- Umbrales (dependientes de la medida) para el baseline plausible.
    ...
    -- Baseline propio SOLO con echadas plausibles (no envenenar con basura de
    -- corta distancia ni outliers imposibles).
    ...
    select avg(rendimiento_km_gal) into v_prom_flota
      from sgc.registros_combustible
     where rendimiento_km_gal is not null and coalesce(es_prueba, false) = false
       and km_recorridos >= v_dist_min;
...
-- Recálculo del histórico con las reglas nuevas (admin). Baseline plausible por
-- vehículo (excluye la propia fila). Devuelve cuántas filas reclasificó.
...
declare
  ...
  v_estado text; v_motivo text;
begin
  if not sgc.is_admin() then raise exception 'Solo un administrador puede recalcular el histórico'; end if;
  for r in
    select id, vehiculo_id, km_recorridos, galones, rendimiento_km_gal
      from sgc.registros_combustible
     where vehiculo_id is not null
     order by vehiculo_id, kilometraje
  loop
...
grant execute on function sgc.recalcular_estados_combustible() to authenticated;

-- set_flota_config — para que Administración/Flota edite los umbrales (Hard-rule #2).
...
begin
  if not (sgc.is_admin() or sgc.tiene_modulo('flota')) then
    raise exception 'No tienes permiso para cambiar la configuración de flota';
  end if;
  insert into sgc.flota_config (clave, valor) values (p_clave, p_valor)
