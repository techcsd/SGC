-- ============================================================================
-- PROMPT-44 (BO) FASE 1 — BO4 + BO5: el gate del registro de combustible ya no
-- es "solo admin", es "flota elevado".  Ronda 13/09/2026.  Aditivo, idempotente.
--
-- CONTEXTO.  registrar_combustible_app tiene DOS decisiones que hoy sólo el admin
-- puede tomar, y eso deja fuera a quien realmente opera la flota (Raykler = rol
-- 'logistica', gerencia, jefe de flota):
--   1) BO4 — el gate de autorización "ese vehículo no es tuyo" (bm1:161) sólo se
--      salta con sgc.is_admin().  → pasa a sgc.es_flota_elevado(), que cubre
--      admin, direccion, gerencia, jefe_flota y logistica (Raykler).  Así un
--      referente de flota puede registrar la echada de cualquier vehículo, que es
--      exactamente lo que pidió Xaviel (admin + gerencia + Raykler).
--   2) BO5 — el salto de kilometraje (bm1:217): hoy sólo el admin puede "pasar"
--      un salto > umbral (deja km_alerta=true para revisión); todos los demás
--      reciben rechazo corregible.  → mismo helper, para que un elevado también
--      pueda dejarlo marcado para revisión en vez de quedar bloqueado.
--
-- IMPORTANTE.  La versión VIVA del RPC NO es aw3-combustible-rpcs.sql; es
-- 2026-09-09-bm1-combustible-canal-negocio.sql (create or replace sobre la misma
-- firma de 20 args).  Esta migración es copia EXACTA de bm1 con SÓLO esos dos
-- is_admin() → es_flota_elevado().  Nada más cambia: ni reglas, ni flujo, ni el
-- canal de error (regla 9 intacta).  Server-side ⇒ web y app se benefician sin
-- release (patrón BK).
--
-- NO se tocan las otras dos guardas de negocio (capacidad de tanque bm1:133 y
-- banda de precio bm1:147): hoy NO tienen bypass de admin y esta migración no se
-- lo añade — eso lo decide Xaviel (§E-4).  Tampoco se toca el fallback a
-- vehiculos.responsable_id (bm1:170): es el "puente legacy" de vehiculo_asignaciones
-- (regla 12) y su saneamiento es decisión aparte (§E-4).
--
-- Apply: node scripts/apply-migration.mjs sql/2026-09-13-bo4-bo5-combustible-gate-flota-elevado.sql
-- ============================================================================
