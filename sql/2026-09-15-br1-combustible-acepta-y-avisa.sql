-- BR1 + BQ7 (causa real) — El combustible ACEPTA, MARCA y AVISA (regla 15).
-- Nota #28: "el kilometraje de la última echada… input que a mí (y solo a mí como admin)
--   me salga al editar un vehículo… varias echadas que no se registraron y el sistema no
--   permite… Raykler y admin puedan modificar eso o registrar esa echada, sin impedir nada".
-- Nota #14: "si un chofer olvida registrar combustible… al otro no lo deja por la alerta de km irreal".
-- Nota #15: "solo el usuario asignado… eso hay que cambiarlo".
-- Nota #23: "Felix… no le deja registrar echadas… que todos puedan trabajar".
--
-- Regla 15 (checklist 15ª): el servidor no le impide a un usuario registrar lo que hizo:
--   lo acepta con bandera, avisa a quien puede resolverlo, y el rechazo duro queda solo
--   para lo físicamente imposible (galones > tanque, precio fuera de banda) — y esos dos
--   los confirma un flota-elevado.
--
-- Aditivo y retrocompatible. registrar_combustible_app es copia VIVA (pg_get_functiondef)
-- con cambios quirúrgicos marcados BR1.

begin;

-- 1) Columnas ---------------------------------------------------------------

-- Bandera de echada aceptada sin coincidir con la asignación (AF18, regla 15).
alter table sgc.registros_combustible
  add column if not exists sin_asignacion boolean not null default false;

-- Km base de combustible editable por admin: reinicia el punto desde el que se
-- mide el salto, sin tocar las echadas históricas (no falsea el kardex).
alter table sgc.vehiculos
  add column if not exists km_base_combustible integer,
  add column if not exists km_base_combustible_at timestamptz,
  add column if not exists km_base_combustible_por uuid references sgc.usuarios(id);

-- 2) flota_config — umbrales del cron (regla 7: interruptor consultado) ----------
insert into sgc.flota_config (clave, valor)
values ('dias_actividad_echada', 7), ('dias_sin_echada_aviso', 10)
on conflict (clave) do nothing;

-- 3) notif_tipo — tipos nuevos (canal in-app + push; correo OFF por default) -----
insert into sgc.notif_tipo (tipo, etiqueta, descripcion, es_operativa, canales, activo, orden)
values
  ('km_salto', 'Salto de kilometraje', 'Una echada registró un salto de km mayor al umbral (revisar/sanear).', false, array['in_app','push'], true, 60),
  ('combustible_revisar', 'Echada sin asignación', 'Se registró una echada por alguien que no es el asignado del vehículo.', false, array['in_app','push'], true, 61),
  ('vehiculo_sin_echada', 'Vehículo sin echada', 'Un vehículo con actividad reciente lleva días sin registrar combustible.', false, array['in_app','push'], true, 62)
on conflict (tipo) do nothing;

-- 4) RPC — km base editable por admin (nota #28: "solo a mí como admin") ---------
create or replace function sgc.vehiculo_set_km_base_combustible(
  p_vehiculo_id uuid, p_km integer, p_motivo text)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  if not sgc.is_admin() then
    raise exception 'Solo un administrador puede editar el km base de combustible' using errcode = '42501';
  end if;
  if p_km is null or p_km < 0 then
    raise exception 'El km base debe ser un número válido' using errcode = '22023';
  end if;
  if not exists (select 1 from sgc.vehiculos where id = p_vehiculo_id) then
    raise exception 'Vehículo no encontrado';
  end if;
  select km_base_combustible into v_antes from sgc.vehiculos where id = p_vehiculo_id;

  update sgc.vehiculos
     set km_base_combustible = p_km,
         km_base_combustible_at = now(),
         km_base_combustible_por = v_uid
   where id = p_vehiculo_id;

  insert into sgc.audit_log (actor_id, action, metadata)
  values (v_uid, 'km_base_combustible',
    jsonb_build_object('vehiculo_id', p_vehiculo_id, 'antes', v_antes, 'despues', p_km, 'motivo', nullif(p_motivo,'')));

  return jsonb_build_object('ok', true, 'km_base', p_km);
end $function$;

grant execute on function sgc.vehiculo_set_km_base_combustible(uuid, integer, text) to authenticated;

-- 5) RPC — lectura del punto de medición (precarga el campo del drawer) ----------
create or replace function sgc.vehiculo_km_ultima_echada(p_vehiculo_id uuid)
returns jsonb
language plpgsql
security definer
set search_path to 'sgc','pg_temp'
...
declare
  ...
  v_base int;
begin
  select max(kilometraje) into v_max
    from sgc.registros_combustible
   where vehiculo_id = p_vehiculo_id and kilometraje is not null
     and not coalesce(es_prueba, false) and not coalesce(invalidada, false);
  select km_base_combustible into v_base from sgc.vehiculos where id = p_vehiculo_id;
  return jsonb_build_object(
    'max_echada', v_max,
    'km_base', v_base,
    'efectivo', nullif(greatest(coalesce(v_max,0), coalesce(v_base,0)), 0));
end $function$;

grant execute on function sgc.vehiculo_km_ultima_echada(uuid) to authenticated;

-- 6) registrar_combustible_app — copia VIVA con los 3 cambios BR1 ----------------
...
-- 6b) log_combustible — exponer la bandera sin_asignacion al registro web ---------
-- (RETURNS TABLE cambia → drop + recreate).
...
-- 7) Cron — vehículos con actividad reciente y sin echada (BO §E-4, la causa) -----
-- Regla 11: declarada en sql/. Documentada en docs/CRONS.md.
create or replace function sgc.avisar_vehiculos_sin_echada()
returns integer
language plpgsql
security definer
set search_path to 'sgc','pg_temp'
as $function$
declare
  v_dias_act  int := coalesce((select valor::int from sgc.flota_config where clave='dias_actividad_echada'), 7);
  v_dias_sin  int := coalesce((select valor::int from sgc.flota_config where clave='dias_sin_echada_aviso'), 10);
  v_semana    text := to_char(current_date, 'IYYY-IW');
  v_rec       record;
  v_n         int := 0;
begin
  for v_rec in
    select v.id, v.placa
     ...
     where coalesce(v.activo, true) and not coalesce(v.es_prueba, false)
       and coalesce(v.estado,'activo') <> 'baja'
       and (
            exists (select 1 from sgc.vehiculo_usos u
                     where u.vehiculo_id = v.id and not coalesce(u.es_prueba,false)
                       and u.inicio_at >= now() - make_interval(days => v_dias_act))
         or exists (select 1 from sgc.rutas r
                     where r.vehiculo_id = v.id and not coalesce(r.es_prueba,false)
                       and coalesce(r.fecha, r.iniciada_at::date) >= current_date - v_dias_act)
       )
       and not exists (
            select 1 from sgc.registros_combustible g
             where g.vehiculo_id = v.id and not coalesce(g.es_prueba,false)
               and not coalesce(g.invalidada,false)
               and g.fecha >= current_date - v_dias_sin)
  loop
    -- dedup: una vez por vehículo/semana (uq_avisos_flota_dedup).
    insert into sgc.avisos_flota (tipo, vehiculo_id, mensaje, severidad, dedup_key)
    values ('vehiculo_sin_echada', v_rec.id,
      format('%s tiene actividad reciente pero sin echada de combustible en %s+ días.', coalesce(v_rec.placa,'Un vehículo'), v_dias_sin),
      'media', 'sin_echada:' || v_rec.id::text || ':' || v_semana)
    on conflict (dedup_key) do nothing;

    if found then
      perform sgc.notificar_modulo('flota', 'vehiculo_sin_echada',
        'Vehículo sin echada reciente',
        format('%s: %s+ días sin registrar combustible pese a tener actividad.', coalesce(v_rec.placa,'Un vehículo'), v_dias_sin),
        '/flota/combustible-log', v_rec.id, 'vehiculo');
      v_n := v_n + 1;
    end if;
  end loop;
  return v_n;
...
-- 8) Cron schedule (07:00 RD = 11:00 UTC). Idempotente. -------------------------
select cron.unschedule('sgc-vehiculos-sin-echada')
 where exists (select 1 from cron.job where jobname = 'sgc-vehiculos-sin-echada');
select cron.schedule('sgc-vehiculos-sin-echada', '0 11 * * *', $$select sgc.avisar_vehiculos_sin_echada()$$);
