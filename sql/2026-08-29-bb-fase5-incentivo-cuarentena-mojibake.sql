-- ════════════════════════════════════════════════════════════════════════════
-- BB (PROMPT-19 FASE 5) — Desempeño de choferes:
--   (BB8d) MOJIBAKE erradicado en la raíz: los textos de incidencia se reescriben
--          con el guion largo correcto (—). AX4/AX4b/AX5 se guardaron con UTF-8
--          doble-codificado (secuencia rota en vez del guion largo) y AX5 es la definición viva.
--   (BB8b) CUARENTENA (decisión de Xaviel): una ruta/echada con incidencia NO
--          puntúa hasta que alguien la ACEPTE. Hoy 30 rutas de 0 km inflaban a Joan
--          (32/10). Sin aceptación → no suma; 'excluida' → tampoco; 'aceptada' → suma.
--   Nueva tabla de decisiones por incidencia + RPC auditado + read path con estado.
-- Aditivo, idempotente, retrocompatible. Reutiliza el motor AT1 y su versionado.
-- ════════════════════════════════════════════════════════════════════════════

set search_path = sgc, public;

-- ── 1) Decisiones por incidencia (cuarentena / aceptada / excluida) ───────────
create table if not exists sgc.incentivo_incidencia_decision (
  id           uuid primary key default gen_random_uuid(),
  anio         int  not null,
  semana       int  not null,
  ref_tipo     text not null check (ref_tipo in ('ruta','echada')),
  ref_id       uuid not null,
  decision     text not null check (decision in ('aceptada','excluida')),
  motivo       text,
  decidido_por uuid references sgc.usuarios(id),
  decidido_at  timestamptz not null default now(),
  unique (anio, semana, ref_tipo, ref_id)
);
comment on table sgc.incentivo_incidencia_decision is
  'BB8b — decisión humana por incidencia de una ruta/echada. Sin fila = cuarentena (no puntúa). aceptada = puntúa; excluida = no puntúa (resuelta).';
create index if not exists ix_inc_incidencia_dec on sgc.incentivo_incidencia_decision (anio, semana);

alter table sgc.incentivo_incidencia_decision enable row level security;
do $$ begin
  create policy inc_incidencia_dec_sel on sgc.incentivo_incidencia_decision
    for select using (sgc.puede_gestionar_incentivos());
exception when duplicate_object then null; end $$;
grant select, insert, update on sgc.incentivo_incidencia_decision to authenticated, service_role;

-- ── 2) Motor: cuarentena + mojibake corregido ────────────────────────────────
-- Regla: una ruta/echada con incidencia solo cuenta para el puntaje si tiene una
-- decisión 'aceptada'. Las flags se conservan en la salida (con su estado) para que
-- la UI las agrupe y ofrezca aceptar/excluir.
...
-- ── 3) RPC: decidir una incidencia (aceptar / excluir), auditado + recomputa ──
create or replace function sgc.incentivo_decidir_incidencia(
  p_anio integer, p_semana integer, p_ref_tipo text, p_ref_id uuid,
  p_decision text, p_motivo text default null
) returns void
...
begin
  ...
  if p_ref_tipo not in ('ruta','echada') then
    raise exception 'Tipo de incidencia inválido' using errcode = 'BB400';
  end if;
  ...
  values (p_anio, p_semana, p_ref_tipo, p_ref_id, p_decision, nullif(trim(p_motivo),''), auth.uid())
  ...
  -- Recalcula la semana para que el puntaje refleje la decisión de inmediato.
  perform sgc.incentivo_generar_semana(p_anio, p_semana);
end;
$$;
grant execute on function sgc.incentivo_decidir_incidencia(integer, integer, text, uuid, text, text) to authenticated, service_role;

-- ── 4) Recompatibilidad de datos: reponer la semana actual/pasada re-genera con
-- la nueva lógica cuando el cron o un admin lo dispare (no forzamos aquí).
