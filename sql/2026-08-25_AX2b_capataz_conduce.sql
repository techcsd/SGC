-- ============================================================================
-- AX2 (parte 2) — El CAPATAZ de la obra destino puede VER y FIRMAR el conduce que
-- recibe (mismo principio que AX1 para el ingeniero responsable: acción = lectura,
-- una sola matriz). Un capataz se vincula a su obra por su ficha de personal
-- (sgc.personal_obra: usuario_id + proyecto_id + cargo Capataz, activo).
-- Aditivo y retrocompatible.
-- ============================================================================

-- Helper: ¿el usuario es capataz (con acceso) de esta obra?
CREATE OR REPLACE FUNCTION sgc.es_capataz_de_proyecto(p_proyecto_id uuid, p_usuario uuid DEFAULT auth.uid())
 RETURNS boolean
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'sgc', 'pg_temp'
AS $function$
  select p_proyecto_id is not null and p_usuario is not null and exists (
    select 1
      from sgc.personal_obra po
      join sgc.cargos c on c.id = po.cargo_id
     where po.usuario_id = p_usuario
       and po.proyecto_id = p_proyecto_id
       and c.codigo = 'CAP'
       and coalesce(po.estado, 'activo') <> 'inactivo'
  );
$function$;

-- Lectura del conduce para el capataz de la obra destino (cabecera/items/firmas/libres).
drop policy if exists "salidas_inventario: select capataz" on sgc.salidas_inventario;
create policy "salidas_inventario: select capataz" on sgc.salidas_inventario
  for select using (sgc.es_capataz_de_proyecto(proyecto_id));

drop policy if exists "detalle_salidas: select capataz" on sgc.detalle_salidas;
create policy "detalle_salidas: select capataz" on sgc.detalle_salidas
  for select using (exists (
    select 1 from sgc.salidas_inventario si
    where si.id = detalle_salidas.salida_id
      and sgc.es_capataz_de_proyecto(si.proyecto_id)));

drop policy if exists "salida_firmas: select capataz" on sgc.salida_firmas;
create policy "salida_firmas: select capataz" on sgc.salida_firmas
  for select using (exists (
    select 1 from sgc.salidas_inventario si
    where si.id = salida_firmas.salida_id
      and sgc.es_capataz_de_proyecto(si.proyecto_id)));

drop policy if exists "salida_items_libres: select capataz" on sgc.salida_items_libres;
create policy "salida_items_libres: select capataz" on sgc.salida_items_libres
  for select using (exists (
    select 1 from sgc.salidas_inventario si
    where si.id = salida_items_libres.salida_id
      and sgc.es_capataz_de_proyecto(si.proyecto_id)));

-- Confirmación: el capataz de la obra destino también autoriza (rama añadida).
...
begin
  ...
  select sgc.is_admin() or sgc.tiene_modulo('inventario')
    ...
    -- AX2 — el capataz de la obra destino también recibe/firma.
