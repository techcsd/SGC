-- BQ7 — Blindaje del conductor_id en registrar_combustible_app  ·  Ronda 14/09/2026
-- ---------------------------------------------------------------------------------
-- CAUSA (corolario de la regla 13/14 del checklist).  BP1 (2026-09-14) fusionó y
-- BORRÓ el conductor fantasma de Felix (id 0dbc9b02-4974-403c-80ae-53f8d3db4321).
-- El outbox del teléfono de Felix tiene echadas encoladas cuyo payload lleva ese
-- conductor_id fantasma.  El RPC INSERTA `p_conductor_id` directo en
-- `registros_combustible.conductor_id` (FK → sgc.conductores) → al reintentar da
-- 23503 (FK), clase 'sistema' → el outbox queda "Con problema al enviar" para
-- siempre.  Verificado en prod (14/09): fantasma ausente, 0 filas lo referencian
-- (BP1 fue limpio), una sola sobrecarga viva (20 args) con EXECUTE a authenticated,
-- p_conductor_id ES el 3er arg → la app manda el id cacheado.
--
-- ARREGLO (aditivo, quirúrgico).  Copia VIVA del RPC (pg_get_functiondef, la versión
-- bo4-bo5 con es_flota_elevado) + UN bloque nuevo: nunca insertamos un conductor_id
-- que el cliente no pueda respaldar.  El servidor lo resuelve por auth.uid().  Así
-- los 3 envíos de Felix pasan AL REINTENTAR, sin tocar el teléfono (§F-2 = reintento
-- automático).  conductor_id es NULLABLE (verificado) → resolver a null es seguro.
--
-- Validar: begin; \i este.sql; select ... ; rollback;   Aplicar solo con OK de Xaviel.
-- ---------------------------------------------------------------------------------
...
begin
  ...
  -- BQ7 — Blindaje del conductor_id (corolario regla 13/14).  Nunca insertamos un
  -- conductor_id que el cliente no pueda respaldar: un payload encolado antes de una
  -- fusión (BP1) puede traer un id ya borrado → 23503 (sistema) y outbox atascado.
  --   1) si el id del payload no existe en conductores, lo ignoramos (raise notice);
  --   2) si queda nulo, lo resolvemos por auth.uid() (la ficha del que registra);
  --   3) si aún nulo y hay vehículo, por su conductor asignado activo.
  ...
  if not v_persona then
    ...
    -- AF18 — solo el usuario asignado registra en su vehículo.
    -- BO4 — bypass ampliado: de is_admin() a es_flota_elevado() (admin, direccion,
    -- gerencia, jefe_flota, logistica=Raykler).  Un referente de flota registra la
    -- echada de cualquier vehículo.
    ...
    if v_km_anterior is not null then
      ...
      if v_medida <> 'horas' then
        ...
        if v_km_recorridos > v_umbral_km then
          -- BO5 — antes: sólo is_admin() podía "pasar" el salto (deja km_alerta
          -- para revisión).  Ahora: cualquier flota elevado (incluye Raykler).
          if sgc.es_flota_elevado() then
            v_km_alerta := true;
          else
            -- BM1 — rechazo de negocio, campo corregible → canal de datos (22023).
...
grant execute on function sgc.registrar_combustible_app(uuid,uuid,uuid,date,integer,numeric,numeric,text,text,text,text,text,text,text,text,boolean,text,text,uuid,boolean) to authenticated;
