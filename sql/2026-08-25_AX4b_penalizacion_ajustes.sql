-- ============================================================================
-- AX4b - Ajustes a la penalizacion por estancamiento (decisiones de Xaviel):
--   * Semana COMPLETA: los choferes trabajan tambien domingos -> sin exencion.
--   * Poblacion = TODOS los choferes activos (aunque no tengan actividad), para
--     que el chofer totalmente estancado tambien entre a la matriz y se penalice.
-- Aditivo/retrocompatible; reemplaza las dos funciones.
-- ============================================================================
...
begin
  ...
  , choferes as (
    -- AX4b - todos los choferes activos (rol + registro de conductor activo).
    select distinct u.id as usuario_id
      from sgc.usuarios u
      join sgc.usuarios_roles ur on ur.usuario_id = u.id
      join sgc.roles r on r.id = ur.rol_id and r.codigo = 'chofer_transportista'
      join sgc.conductores c on c.usuario_id = u.id and coalesce(c.activo, true)
  )
  ...
  select p_anio, p_semana, v_inicio, v_fin, ch.usuario_id,
         (select id from sgc.conductores c where c.usuario_id = ch.usuario_id limit 1),
    ...
    from choferes ch
    left join agg a on a.usuario_id = ch.usuario_id
    left join flags_all fa on fa.usuario_id = ch.usuario_id
