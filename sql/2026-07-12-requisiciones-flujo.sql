-- ============================================================================
-- A2 — Flujo unificado de Requisición con auto-división (reunión 07/07/2026)
-- ----------------------------------------------------------------------------
-- La "solicitud de material" ES la Requisición. Al aprobar, el sistema divide:
--   * lo que hay en stock del almacén elegido -> DESPACHO (salida / conduce)
--   * el faltante -> SOLICITUD DE COMPRA automática (bandeja de Compras)
-- Reutiliza registrar_salida_inventario y el flujo de aprobación de Compras.
--
-- 100% aditivo y retro-compatible:
--   * NO se toca aprobar_solicitud_material (queda deprecada, se conserva >=2 vers).
--   * NO se toca crear_solicitud_app (app de campo).
--   * Los estados se mantienen dentro del set conocido por la app:
--       pendiente | aprobada | rechazada | entregada
-- ============================================================================

set search_path = sgc, public;

-- 1) Enlaces de trazabilidad -------------------------------------------------
alter table sgc.solicitudes_material
  add column if not exists solicitud_compra_id uuid references sgc.solicitudes_compra(id),
  add column if not exists bodega_id uuid references sgc.bodegas(id);

alter table sgc.solicitudes_compra
  add column if not exists origen_requisicion_id uuid references sgc.solicitudes_material(id);

comment on column sgc.solicitudes_material.solicitud_compra_id is
  'Requisición: solicitud de compra auto-generada por el faltante al aprobar (A2).';
comment on column sgc.solicitudes_material.bodega_id is
  'Requisición: almacén desde el que se despachó la parte en stock (A2).';
comment on column sgc.solicitudes_compra.origen_requisicion_id is
  'Compra originada por el faltante de una requisición aprobada (A2). Nada llega a Compras sin requisición.';

-- 2) Aprobación unificada con auto-división ----------------------------------
create or replace function sgc.aprobar_requisicion(
  ...
  p_items         jsonb   -- [{articulo_id, descripcion, unidad, cantidad}]
) returns jsonb
language plpgsql
security definer
...
begin
  ...
  -- Despacho de la parte en stock (valida stock server-side; nunca excede)
  ...
  -- Solicitud de compra automática por el faltante
...
-- 3) Realtime para badges en vivo e interconexión (aditivo y seguro) ---------
do $$ begin
  alter publication supabase_realtime add table sgc.solicitudes_material;
exception when duplicate_object then null; end $$;
do $$ begin
  alter publication supabase_realtime add table sgc.solicitudes_compra;
exception when duplicate_object then null; end $$;
do $$ begin
  alter publication supabase_realtime add table sgc.salidas_inventario;
exception when duplicate_object then null; end $$;
