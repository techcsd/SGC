...
- **Primario — correo (Resend)**: `from` = `NOTIFICATIONS_FROM_EMAIL` (`notificaciones@sgcconstructorasd.com`, ya verificado). Destinatarios = `INFRA_ALERT_EMAILS` (lista separada por coma, **debe incluir al menos un Gmail externo**); si no está seteada, cae a los correos de usuarios admin/tecnología (`usuarios_con_modulo('tecnologia')`). Resend key desde Vault; no-op si falta.
  - ⚠️ **Pendiente menor de Xaviel:** darme un correo EXTERNO (Gmail) para setear `INFRA_ALERT_EMAILS` y así resistir el caso "dominio corporativo caído". Sin eso, las alertas igual llegan a `tecnologia@constructorasd.com` en el 95% de los casos (todo salvo que el propio correo corporativo esté caído).
...
**Canal primario = correo** (Resend desde `sgcconstructorasd.com`, independiente del dominio vigilado). Verificado 28/07: NXDOMAIN de prueba → alerta `critica` → **correo enviado** (`notified_channels: email ok=true`) a `tecnologia@constructorasd.com`. Telegram quedó OPCIONAL (solo si se setean sus secrets). **Pendiente menor:** setear `INFRA_ALERT_EMAILS` con un Gmail externo para resistir el caso "correo corporativo caído".
