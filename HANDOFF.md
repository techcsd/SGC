...
## TL;DR — PROMPT-52 (Ronda BR) — 15/09/2026 — **SHIPPED: web 1.133.0 (commit push main). 7 migraciones APLICADAS + verificadas por objeto en prod, edge `acceso-cedula` v7 desplegada, usuario "El Flaco" CREADO. Toda la ronda BR entregada.**

**SHIPPED (aplicado a prod esta sesión):** las 7 migraciones `sql/2026-09-15-*` aplicadas + verificadas por objeto (`sin_asignacion`, `km_base_combustible`, `salidas_inventario.responsable_id`, `rechazar_recepcion`, `importar_proveedores`, tabla `cartillas` + 5 diámetros sembrados + bucket `sgc-cartillas`, crons `sgc-vehiculos-sin-echada`/`sgc-requisiciones-vencidas`, CHECK de salidas con `rechazada`). Edge `acceso-cedula` → **v7** (tipo `encargado`). **El Flaco creado** (id `2fb18263-c0ef-44d6-beb3-88f839fcea3a`, email `e-00000000000@acceso.constructorasd.local`, rol *Encargado de Patio y Bodega Central*, encargado de Bodega Central, SIN ficha de conductor — regla 13 OK).
...
- **Residuales web CERRADOS en 1.134.0** (commit follow-up, aplicado a prod): F3 user-picker en *Nueva salida/conduce*; F4 guard servidor de entrada 0-renglones (`sql/2026-09-15-br4b-…`, aplicada); F8 figuras SVG (`shared/ui/cartilla-figura-icono`) + fotos/plano en la captura web + Admin *Catálogos* (kg/m + figuras, `sql/2026-09-15-br-bo10-catalogos-admin.sql` aplicada); F10 filter-select de estado en Almacenes + enlazar issue/versión desde el checklist de notas. **Residual mínimo restante:** filtrar a `es_tecnologia` el selector de compartir de dev-notes (la RLS ya lo protege). App = PROMPT-53 (BR6 Descartar, BR7 idioma, F4/F5 paridad).
