...
(Mensajería, "Mis tareas", **Notas** y **Configuración** no requieren módulo — todo usuario autenticado.)
**BS3 — Configuración (`/configuracion`): SIN gate de módulo** (solo `authGuard`, como Notas/Mensajes). Es preferencia personal: cuenta, idioma, apariencia, notificaciones, pantalla de inicio, sesión/dispositivos, privacidad (choferes) y acerca de. `/perfil` y `/ajustes/notificaciones` redirigen ahí (deep-links vivos).
...
**⭐ Regla 16 (checklist de migraciones, BS1/BS2) — lo que el usuario ve lo decide su ROL, no el estado del sistema.** Un **error técnico nunca le habla al usuario en lenguaje de desarrollador**: se traduce (mensaje por rol), se reporta a Tecnología con el detalle (ruta/rol/SQLSTATE), y el **detalle técnico solo lo ve `es_desarrollador()`** (`shared/ui/error-state` + `friendly-error.util.presentarError`; guard de build `verify-dev-strings.mjs`). **Corolario:** un **selector nunca esconde opciones válidas por conveniencia** — la conveniencia es el preseleccionado, no el filtro (BS1: el almacén de despacho ofrece todos los que el rol lee, Central preseleccionado). **Corolario de acceso:** si una cuenta ve algo que no debería, se arregla del lado que corresponda (guard = RLS); DEFAULT si es ambiguo = no se abre la RLS. Costo de olvidarlo: BS2 — a Raykler (almacén) le salía "ejecuta el SQL en el SQL Editor de Supabase"; la RLS ya le concedía Flota, el bug era el banner de desarrollador.
