...
10. **⭐ BS2 (regla 16) — lo que el usuario ve lo decide su ROL, no el estado del sistema**: un error técnico nunca le habla al usuario en lenguaje de desarrollador — se traduce (mensaje por rol), se reporta a Tecnología con el detalle, y el detalle técnico solo lo ve `es_desarrollador()` (`shared/ui/error-state` + `friendly-error.util.presentarError`; guard `verify-dev-strings.mjs`). Un selector nunca esconde opciones válidas por conveniencia: la conveniencia es el preseleccionado, no el filtro.
11. **⭐ BS4 — i18n con `t()`**: la web tiene i18n runtime (`shared/i18n`, portado del hijo csd-app). Las claves de traducción SON el texto en español (funciona sin catálogo). **Toda pantalla que se toque pasa por `t()`** (`{{ 'Texto' | t }}` / `i18n.t('Texto')`), y su clave `en` se agrega a `public/i18n/en.json` (rollout incremental; el guard `verify-i18n.mjs` avisa cobertura). El idioma canónico vive en `usuarios.idioma`.
12. **⭐ BU1 (regla 18) — nada llega a prod sin pasar por dev**: hay dos entornos (prod `jeeqhgccqefbqilntcpu` / **dev `fzfrnrvndzrjwyvdpkgg` = sgc-dev**). Toda migración/edge/secret/versión se aplica **primero a dev** (`--env dev`), se prueba en `dev.sgcconstructorasd.com`, y solo entonces a prod. Los scripts lo obligan: **sin `--env` no corren**, y `--env prod` **rechaza** lo que no esté en el ledger de dev (salvo `--force-prod --motivo "…"`, que queda registrado). Flujo: `feature/*` → `dev` → PR `dev → main`. `environment.ts` local está gitignored (`npm run env:dev`). Nunca hardcodees el ref de prod (guard `verify-sin-ref-hardcodeado`). **Todo el detalle + pasos físicos en [`docs/ENTORNOS.md`](./docs/ENTORNOS.md).**
