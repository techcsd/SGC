...
// avance físico pesa poco porque una obra recién arrancada NO se desempeña mal por
// estar temprano — antes el avance (0.30) volvía rojo a todo proyecto nuevo.
const PESO_AVANCE = 0.15;
const PESO_BITACORA = 0.3;
const PESO_SEGURIDAD = 0.3;
const PESO_PRESUPUESTO = 0.25;

// AY3 — meta de bitácoras realista: ~3 reportes/semana en 30 días ≈ 12 días (antes
// 20 = reporte casi diario, un piso demasiado alto que hundía el score).
const BITACORA_META_30D = 12;
// Each incident in the last 90 days knocks this many points off the safety score.
const PENALIZACION_INCIDENTE = 25;

// AY3 — umbrales de la escala de color (verde ≥80 / amarillo 60-79 / rojo <60).
const UMBRAL_BUENO = 80;
const UMBRAL_MEDIO = 60;
...
export class Kpi implements OnInit {
  ...
  scoreBars = computed<BarDatum[]>(() =>
    this.ranking().map((k) => ({
      ...
      key: k.proyecto_id,
      color:
        k.scoreTotal >= UMBRAL_BUENO ? 'var(--sgc-success)' : k.scoreTotal >= UMBRAL_MEDIO ? 'var(--sgc-warning)' : 'var(--sgc-danger)',
    })),
  );
  ...
  desempenoDonut = computed<DonutDatum[]>(() => {
    ...
    return [
      { label: 'Buen desempeño (80+)', value: r.filter((k) => k.scoreTotal >= UMBRAL_BUENO).length, color: 'var(--sgc-success)' },
      { label: 'Aceptable (60-79)', value: r.filter((k) => k.scoreTotal >= UMBRAL_MEDIO && k.scoreTotal < UMBRAL_BUENO).length, color: 'var(--sgc-warning)' },
      { label: 'Necesita atención (<60)', value: r.filter((k) => k.scoreTotal < UMBRAL_MEDIO).length, color: 'var(--sgc-danger)' },
    ];
  });
  ...
  scoreClass(score: number): string {
    if (score >= UMBRAL_BUENO) return 'score--good';
    if (score >= UMBRAL_MEDIO) return 'score--mid';
    return 'score--low';
  }
